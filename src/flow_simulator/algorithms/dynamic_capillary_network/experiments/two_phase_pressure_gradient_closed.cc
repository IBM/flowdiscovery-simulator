/**
 * \file src/flow_simulator/algorithms/dynamic_capillary_network/experiments/two_phase_pressure_gradient_closed.cc
 * \brief Contains the implementation of \c TwoPhasePressureGradientClosed class methods.
 *
 ...
 * \date 2018
 *
 * This source file contains the implementation of \c TwoPhasePressureGradientClosed class methods.
 */

#include "src/flow_simulator/algorithms/dynamic_capillary_network/experiments/two_phase_pressure_gradient_closed.h"
#include <glog/logging.h>
#include <algorithm>

namespace simulator {

arma::Col<double> TwoPhasePressureGradientClosed::CalculateLinearPressure(
  const arma::Col<double> &u_coords,
  const double midpoint,
  const double voxel_size) const {
/**
 * The \c TwoPhasePressureGradientClosed::CalculateLinearPressure() method returns a pressure value as
...
}  // end of TwoPhasePressureGradientClosed::CalculateLinearPressure() method



arma::Col<double> TwoPhasePressureGradientClosed::ApplyBoundaryConditions(
  std::shared_ptr<CapillaryInterface> interface_) {
/**
 * The \c TwoPhasePressureGradientClosed::ApplyBoundaryConditions() calculates a linear pressure
 * gradient and applies the pressure in the inlet and outlet nodes of the sample. The side
 * boundaries are undone, so the zero flow is automatically enforced by the mass conservation
 * equation Then it injects the invading fluid in the capillaries connected to inlet nodes.
 *
 ...
 */
  ...
  // Remove the side boundaries, so mass conservation automatically deals with closed boundaries
  context_->interior_nodes_ = arma::sort(arma::join_cols(context_->interior_nodes_,
                                                         context_->side_boundary_nodes_));
  context_->side_boundary_nodes_.reset();
  ...
  return pressures;
}  // end of TwoPhasePressureGradientClosed::ApplyBoundaryConditions() method
